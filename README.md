...
| AI | LangChain + Chroma + OpenAI SDK（通义千问 DashScope 兼容接口） |
...
```bash
cd backend && ../.venv/bin/python -m pytest tests/ -v   # 30 个用例
```
...
backend/
  ...
  parsers.py   文档解析（txt/md/pdf/docx）
  llm.py       AI 基础设施（client / 向量库 / 查询改写）
  tests/       pytest 测试（30 用例）
frontend/
  app.py       Streamlit 界面
...
- ✅ **v3.0** — 单元测试（30 用例）、代码重构（五模块）、Docker 部署
